...
int main() {
    try {
        ...
        pipeline.set_parallel(false); // Set to 'false' for sequential execution

        // Create thread executor
        ThreadSafeCmdProcessor processor;

        // Push tasks
        processor.push_task(pipeline);

        processor.start_thread();
        

        // Wait for results
        while (!processor.isResultAvailable()) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }

        // Get results
        Result res = processor.pop_result();
